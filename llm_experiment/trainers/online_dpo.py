...
# flags for reward models
flags.DEFINE_bool('use_dpo_reward', False, 'whether to use dpo reward')
...
flags.DEFINE_float('preference_threshold', 0.1, 'the mixing ratio for preference dataset')
...
def main(_):
    ...
    if FLAGS.use_dpo_reward:
        raise NotImplementedError("DPO reward not implemented")
        ref_policy = trainer.ref_model
        def get_dpo_reward(completions, shard_size=1):
            empty_cache()
            if shard_size >= 1:
                scores, beg = [], 0
                with tqdm(total=len(completions), desc="Computing gold scores") as pbar:
                    while beg < len(completions):
                        ...
                        rewards = trainer.config.temperature * (logprobs - old_logprobs).detach()
                ...
                rewards = trainer.config.temperature * (logprobs - old_logprobs).detach()
                return scores
        rew_fn = get_dpo_reward
    elif FLAGS.use_length_reward:
    ...
    def process_dataset(batch):
        ...
        if 'y_w' in batch and 'y_l' in batch:
    ...
    def process_pref_batch(pref_batch):
        ...
        pref_query_tensors = pref_query.to(policy.device)
        ...
        pref_response_w_tensors = tokenized[:len(pref_batch["response_w"])].to(policy.device)
        
        pref_response_l_tensors = tokenized[len(pref_batch["response_w"]):].to(policy.device)
    ...
    @empty_cache_decorator
    @torch.no_grad()
    def process_online_batch_dpo(batch):
        ...
        query_tensors = query_tensors.to(policy.device)
        ...
        for i in range(2):
            if query_tensors.input_ids.shape[0] > FLAGS.max_gen_batch_size:
                ...
                for i in tqdm(range(0, query_tensors.input_ids.shape[0], FLAGS.max_gen_batch_size), desc=f"Generating for epoch {epoch}"):
                    ...
                    torch.cuda.empty_cache()
                max_seq_len = max(generation_tokens, key=lambda x: x.shape[1]).shape[1]
                for i, x in enumerate(generation_tokens):
                    if x.shape[1] < max_seq_len:
                        generation_tokens[i] = tokenizer.pad(x, max_length=max_seq_len, padding='max_length', return_tensors='pt')
        ...
        response1, response2 = all_generation_tokens
        
        text1 = tokenizer.batch_decode(response1, skip_special_tokens=True)
        text2 = tokenizer.batch_decode(response2, skip_special_tokens=True)
        
        rewards1 = rew_fn(text1)
        rewards2 = rew_fn(text2)
        which_better_bool = rewards1 > rewards2
        which_better = which_better_bool.clone().float()
        
        rewards_w = torch.where(which_better_bool, rewards1, rewards2)
        rewards_l = torch.where(which_better_bool, rewards2, rewards1)
        
        preference_mask = (rewards_w - rewards_l) > FLAGS.preference_threshold
        
        #### Update batch with text/response
        for i in range(len(batch['query'])):
            if which_better[i] == 1:
                batch['text_w'][i] = text1[i]
                batch['text_l'][i] = text2[i]
                batch['response_w'][i] = text1[i].split(ASSISTANT_TOKEN)[-1]
                batch['response_l'][i] = text2[i].split(ASSISTANT_TOKEN)[-1]
            else:
                batch['text_w'][i] = text2[i]
                batch['text_l'][i] = text1[i]
                batch['response_w'][i] = text2[i].split(ASSISTANT_TOKEN)[-1]
                batch['response_l'][i] = text1[i].split(ASSISTANT_TOKEN)[-1]
        
        which_better_bool = which_better_bool.to(policy.device)
        response_w_tensors = torch.where(which_better_bool[:, None], response1, response2).to(policy.device)
        response_l_tensors = torch.where(which_better_bool[:, None], response2, response1).to(policy.device)
        assert response_l_tensors.shape[0] == len(batch["response_l"])

        query_tensors = query_tensors.input_ids
        
        return batch, query_tensors, response_w_tensors, response_l_tensors, rewards_w, rewards_l, which_better, preference_mask
    ...
    def process_batch_dpo(batch):
        if batch is not None:
            #### Construct query tensors
            query_tensors = tokenizer(batch["query"], padding='max_length' if FLAGS.use_tpu else True, truncation=True, max_length=128, return_tensors='pt').to(policy.device)
            ...
            for i in range(FLAGS.num_actions_per_prompt): # generate multiple completions per prompt
                if query_tensors.input_ids.shape[0] > FLAGS.max_gen_batch_size:
                    generation_tokens = []
                    for i in tqdm(range(0, query_tensors.input_ids.shape[0], FLAGS.max_gen_batch_size), desc=f"Generating for epoch {epoch}"):
                        subquery_tensors = {k: v[i:i+FLAGS.max_gen_batch_size] for k, v in query_tensors.items()}
                        generation_tokens.append(trainer.model.generate(**subquery_tensors, **generation_kwargs))
                        torch.cuda.empty_cache()
                    max_seq_len = max(generation_tokens, key=lambda x: x.shape[1]).shape[1]
                    for i, x in enumerate(generation_tokens):
                        if x.shape[1] < max_seq_len:
                    ...
                    generation_tokens = trainer.model.generate(**query_tensors, **generation_kwargs)
                all_generation_tokens.append(generation_tokens)
            max_seq_len = max(all_generation_tokens, key=lambda x: x.shape[1]).shape[1]
            for i, x in enumerate(all_generation_tokens):
                if x.shape[1] < max_seq_len:
                    all_generation_tokens[i] = tokenizer.pad(x, max_length=max_seq_len, padding='max_length', return_tensors='pt')
            ...
            response_tensors = response_tensors.to(policy.device)
            
            ### Reprocess query tensors
            query_tensors = query_tensors.input_ids
            # Ensure query and response tensors are same length
            query_tensors = query_tensors.repeat(FLAGS.num_actions_per_prompt, 0)
    ...
    for epoch in tqdm(range(FLAGS.num_train_epochs), desc="Epochs"):
        for sub_iteration, pref_batch in tqdm(enumerate(zipped_dataloaders), desc="Batches", total=total_len):
            ...
            # if total_iterations % FLAGS.eval_every_steps == 1: # for testing
                # TODO: add eval stats
            ...
            pref_batch, pref_query_tensors, pref_response_w_tensors, pref_response_l_tensors, rewards_w, rewards_l, which_better, preference_mask = process_online_batch_dpo(pref_batch)
            
            stats['rewards_w'] = rewards_w
            stats['rewards_w_mean'] = rewards_w.mean().item()
            stats['rewards_w_std'] = rewards_w.std().item()
            stats['rewards_w_max'] = rewards_w.max().item()
            stats['rewards_w_min'] = rewards_w.min().item()
            stats['rewards_l'] = rewards_l
            stats['rewards_l_mean'] = rewards_l.mean().item()
            stats['rewards_l_std'] = rewards_l.std().item()
            stats['rewards_l_max'] = rewards_l.max().item()
            stats['rewards_l_min'] = rewards_l.min().item()
            curr_rewards = torch.cat([rewards_w, rewards_l])
            stats['rewards_mean'] = curr_rewards.mean()
            stats['rewards_std'] = curr_rewards.std()
            stats['rewards_min'] = curr_rewards.min()
            stats['rewards_max'] = curr_rewards.max()
            stats['which_better'] = which_better.mean().item()
            
            if preference_mask is not None:
                # convert to float
                preference_mask = preference_mask.float()
                stats['preference_mask'] = preference_mask
                stats['preference_mask_mean'] = preference_mask.mean().item()
                stats['preference_mask_std'] = preference_mask.std().item()
                stats['preference_mask_max'] = preference_mask.max().item()
                stats['preference_mask_min'] = preference_mask.min().item()
                stats['preference_mask_sum'] = preference_mask.sum().item()
            
            output_batch = {k: pref_batch[k] for k in columns_to_log}

            #### Run Trainer step
            train_stats = trainer.step(queries=pref_query_tensors, responses_w=pref_response_w_tensors, responses_l=pref_response_l_tensors, preference_mask=preference_mask)
                
            for key in train_stats:
                stats[key] = train_stats[key]
                
            rewards = torch.zeros(pref_query_tensors.shape[0], dtype=torch.float32).to(policy.device)
