...
for lr in "${lrs[@]}"; do
    ...
    run_name=reinforce_gold${use_gold_rew}_mix${mix_ratio}_kl${kl_weight}_act${num_actions_per_prompt}_len${length_type}_inner${inner_iteration_step}
    gradient_accumulation_steps=${gradient_accumulation_steps_list[$exp_num % ${#gradient_accumulation_steps_list[@]}]}
    echo "Gradient Accumulation Steps: $gradient_accumulation_steps"
    echo "Run Name: $run_name"
    
    command="python $PWD/trainers/reinforce.py \
