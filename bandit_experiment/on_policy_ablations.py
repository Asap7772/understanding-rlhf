...
def get_arguments():
    ...
    ap.add_argument(
        ...
        type=str,
        default="dpo",
        choices=["dpo", "reinforce", "best_of_n"],
    )
...
def run_algorithm(
    ...
    for num_iteration in num_iterations:
        print(
            f"\nRunning {algorithm} with num iteration: {num_iteration}",
            flush=True,
        )
        training_args = deepcopy(default_hyperparams)

        training_args["num_iterations_on_same_samples"] = num_iteration
        training_args["N"] = training_args["N"] * num_iteration
        num_epochs_recalibrated = int(training_args["num_epochs"] / num_iteration)
        if num_epochs_recalibrated == 0:
            num_epochs_recalibrated += 1
        training_args["num_epochs"] = num_epochs_recalibrated
        ...
        plt.plot(
            ...
            all_reward_list,
            label=f"Iterations={num_iteration}",
        )
    ...
    fig.legend(
        [f"Iterations={num_iteration}" for num_iteration in num_iterations],
        loc='upper center', 
    ...
    lr = default_hyperparams.get("learning_rate")
    savepath = os.path.join(
        filedir,
        f"algorithm={algorithm}_policy={policy}_N={N}_learning_rate={lr}_beta={beta}_gradient_norm={gradient_norm}_normalize={normalize}_negative_gradient_type={negative_gradient_type}.png"
    )
...
def hyperparameter_tuning(
    ...
    if algorithm == "best_of_n":
        ...
        run_algorithm(
            filedir=filedir,
            learner=learner,
            num_iterations=[1, 10, 20, 50, 100, 200],
            default_agent_dict=default_agent_dict,
...
def main():
    ...
    _best_of_n_hyperparams = {
        ...
        "clamp_min": -0.99,
        "clamp_max": 0.99,
        "num_iterations_on_same_samples": None,
    }
